...
async function fetchData() {
  const res = await axiosClient.get(apiEndpoints.mediaFile.getListImage, {
...
<template>
  <div
    class="p-4 lg:p-10 relative bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-100"
  >
    ...
    >
      ...
      >
        ...
        />
        <span
          class="text-sm lg:text-lg font-medium text-white dark:text-gray-200 truncate group-hover:block hidden"
          >{{ image.file_name }}</span
        >
      </div>
      <div
        class="flex items-center gap-2 lg:gap-4 py-2 lg:pl-4 pl-3 lg:py-3 px-2 lg:px-4 hover:bg-blue-600 dark:hover:bg-gray-700 transition-all cursor-pointer group-hover:shadow-lg"
    ...
    >
      ...
      >
        <div class="relative">
          ...
          <span
            ...
            :style="{
              ...
              fontSize: `${form.size / imageRatio}px`,
              top: `${form.position_y / imageRatio}px`,
              left: `${form.position_x / imageRatio}px`,
              fontFamily: fontList.find((font) => font._id === form.font)
                ?.font_name,
            }"
          >
            {{ form.content }}
        ...
        <div class="relative">
          <img
            v-if="selectedImage?.file_watermarked"
            :src="selectedImage?.file_watermarked"
            alt="Watermark Image"
            class="rounded-lg object-contain shadow-md max-w-full lg:max-h-[38vh]"
          />
        </div>
      ...
      >
        <h2
          class="text-lg lg:text-xl font-bold text-gray-700 dark:text-gray-200"
        >
          Edit Image
        </h2>
