...
<template>
  <div class="h-full w-full">
    ...
    >
      ...
      <!-- Page Title with Styled Router Links (Centered) -->
...
/* Add transition for underline hover effect */
.router-link-active,
.router-link-exact-active {
  color: #ffeb3b; /* Bright yellow color when active */
  font-weight: bold; /* Make the active link bold */
}

.router-link-active::before,
.router-link-exact-active::before {
  transform: scaleX(1); /* Ensure underline is fully extended for active link */
}
/* Transition styles for enhanced page transition */
...
.fade-enter-from, .fade-leave-to {
  opacity: 0;
  transform: translateY(10px) scale(0.95); /* Dùng hiệu ứng translate và scale */
}

.fade-enter-to, .fade-leave-from {
  opacity: 1;
  transform: translateY(0) scale(1); /* Trả về vị trí ban đầu và kích thước bình thường */
}
